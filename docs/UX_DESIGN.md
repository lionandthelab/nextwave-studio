...
- **단축키**: 전역 단축키의 **단일 소유자는 `src/ui/shortcuts.ts` 라우터**다(불변식 §10).
  ...
  | `Space` | 재생 / 일시정지 | 재생 |
  | `→` | 노드 하나 실행 (Step) | 재생 |
  | `Esc`(뷰포트) | 정지 (처음으로 되감기) | 재생 |
  | `Ctrl+Z` / `Ctrl+Shift+Z` | 실행 취소 / 다시 실행 | 편집 |
  | `Ctrl+S` | 워크셀 저장 (씬 + 시퀀스) | 파일 |
  | `F` / `Home` | 선택 대상 포커스 / 카메라 리셋 | 카메라 |
  | `Ctrl+Shift+F` / `Ctrl+Shift+D` | 플로우 그래프 / 독 표시 토글 | 보기 |
  | `?` | 도움말 · 단축키 시트 | 일반 |
  | `W`/`E`/`R` | 기즈모 모드 (뷰포트 스코프) | 편집 |
  | `Del` / `Ctrl+D` | 노드 삭제 / 복제 (그래프 스코프) | 편집 |
