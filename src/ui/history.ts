...
export interface SceneHistoryDeps {
   ...
   * 구현한다. false 반환(전환 busy/빌드 실패) 시 이번 스택 이동은 취소된다.
   */
  restore(spec: SceneSpec): Promise<boolean>;
...
export class SceneHistory {
  ...
  private undoStack: SceneSpec[] = [];
  private redoStack: SceneSpec[] = [];
  private pendingTimer: ReturnType<typeof setTimeout> | null = null;
  /** noteChange 시점에 "즉시" 캡처된 스냅샷 — 디바운스 발화/flush 시 커밋된다 */
  private pendingSnapshot: SceneSpec | null = null;
   ...
   */
  noteChange(capture: () => SceneSpec): void {
