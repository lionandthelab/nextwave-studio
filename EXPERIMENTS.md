...
## Phase 12 — 백엔드 · 다중 사용자 · 콘솔 평면 (2026-08-07)

**사용자 결정 3건**: (1) 백엔드 추가 (2) 작업↔씬은 복사본 유지 (3) 다중 사용자.
1차 사용자 페르소나를 **로봇 설치기사**(현장 공유 단말·장갑·IT 담당자 부재)로 고정.

### 결정과 근거

- **서버는 선택적(graceful degradation)**: `/api/v1/health` 실패 시 IndexedDB 로컬
  모드로 강등. 기존 "정적 호스팅 1차 목표" 불변식을 깨지 않으면서 협업을 추가한다.
  현장 네트워크는 불안정하다 — 서버가 죽어도 작업이 계속되는 것이 설치기사 신뢰의 핵심.
- **Fastify + better-sqlite3 단일 파일 DB**: 현장 PC에 "프로세스 1개, 포트 1개,
  DB 파일 1개". Postgres류는 설치기사에게 설치·백업 부담을 전가한다. win32 프리빌드
  확인(Node 22.21). 정적 번들도 같은 프로세스가 서빙(@fastify/static).
- **PIN 타일 로그인**: 공유 단말 전제 — 사용자 타일 + 숫자 PIN(scrypt, 시도 5회 →
  60s 잠금, 세션 토큰은 SHA-256 해시만 저장). 이메일/비밀번호는 현장에서 마찰만 만든다.
  역할 2종(admin/tech), 문서는 사이트 공유 기본 + 감사 추적(created/updated/deleted_by).
- **복사본 의미론 일관 적용**: Task.scene = 공정 씬 전체 사본 + `sceneOrigin{processId,
  processVersion}` 기록. 블록도 참조가 아니라 **삽입 시 전개**(blockRef 흔적만).
  이유: 검증 끝난 작업이 원본 수정으로 조용히 바뀌는 것은 로봇 셀에서 사고다.
  전파는 항상 "알리고 사람이 적용"(불변식 §2.12와 동일 원칙).
- **동시 편집 = 낙관적 버전 + 조언적 잠금**: 저장 시 baseVersion 불일치 → 409 + 서버
  현재본 반환(자동 병합 금지 — 사용자가 서버본/덮어쓰기/사본 선택). 편집 중 표시는
  TTL 90s 잠금(heartbeat). 강탈 없음.
- **Run은 append-only**: 실행 기록은 감사 데이터 — 삭제 API를 만들지 않는다.
  통계는 runCount ≥ 20에서만 노출(작은 표본의 성공률은 노이즈).
- **IA는 9화면 평면 나열이 아니라 2평면**: 콘솔(공정·작업·블록·장비·기록·설정) ↔
  스튜디오(기존 워크스페이스, 편집/실행 모드). 요구 화면 ③④⑧은 스튜디오의 모드다.
  "한 화면에서 완결" 자산을 보존하기 위해 콘솔은 스튜디오 위 전면 레이어로 구현.
- **STATUS 토큰 축 신설**: 액센트 3분할(면/보더/SELECT) 위에 4번째 의미축
  (idle/running/success/warn/error) — 실행 결과·연결 상태 색을 모듈이 발명하지 않게.

### 트레이드오프

- 오프라인 outbox는 순서 보존 재전송 + 충돌 보존(자동 병합 없음)까지만 — CRDT급
  동기화는 범위 밖. 충돌 빈도가 낮은 팀 규모(현장 수 명)에선 수동 해결이 더 예측 가능.
- 실제 장비 연결(`connection.mode:'real'`)은 어댑터 경계만 예약 — UI는 "준비 중"을
  정직하게 표기(회색 버튼 + 사유 title). 유령 기능으로 보이게 두지 않는다.

### Phase 12 구현 결정 (통합 단계에서 확정된 것)

- **RunRecorder는 씬 빌드(main)가 소유한다** — 재생 경로(playbackControls·orchestrator·
  monitor)의 소유자가 기록도 소유해야 개입/충돌/커서가 한 진실에서 나온다. 콘솔 글루는
  submitRun(제출)만 받는다. 기록은 **작업 컨텍스트가 있을 때만**: 데모/프리셋 재생을
  기록하면 기록 화면이 열 수 없는 작업 id로 오염된다.
- **finishRun은 teardownBuilt보다 먼저 선언** — 조립 실패 catch가 teardown을 부를 때
  TDZ가 원인 오류를 가리지 않게. engine 의존은 게터 주입(runSimTimeSec)으로 우회.
- **물리 phase 'stop' → 기록 스키마 'end' 매핑** (CollisionEvent와 runCollisionSchema의
  어휘 차이는 경계에서 1회 변환).
- **loadScene(비-히스토리)이 문서 컨텍스트를 해제한다** — 작업을 연 채 프리셋으로 전환한
  뒤 Ctrl+S가 프리셋 씬으로 작업을 덮어쓰는 사고 방지. bridge.loadDocument는 로드 후
  컨텍스트를 재설정한다(순서가 계약).
- **콘솔 라우트에서는 스튜디오 전역 단축키 전체 정지**(shortcuts.setEnabled(false)) —
  작업 목록에서 Space가 뒤의 재생을 토글하는 유령 조작 방지. 스튜디오 라우트에서는
  워크스페이스 루트를 left:48px로 밀어 얇은 레일이 라이브러리를 덮지 않게 한다.
- **작업 서버 저장은 서버 현재본의 sceneOrigin/notes/thumbnail/assets를 보존**하고
  씬·시퀀스만 교체한다 — 스튜디오는 그 두 축의 진실만 소유한다.
- **현재 PIN 검증은 로그인 API 재사용** — 별도 확인 API를 만들지 않는다(실패 카운터에
  걸리는 것도 의도: 그것이 곧 무차별 대입 방어다).
- **실행 기록 오프라인 대기열은 meta 스토어**(outbox는 개체 4종 전용) — 재연결 flush,
  서버 거부는 1회 폐기+경고(감사 데이터를 무한 재시도로 막지 않는다).
- 화면 에이전트 결정 채택: autoPaused 결과 배지는 STATUS.error(라벨 '충돌 정지') /
  통계 카드 runCount ≥ 20 노출 / 휴지통 보기는 표 밀도 고정 / 블록 삽입 시 robot 명시
  기록(대상 시퀀스 기본 로봇에 조용히 좌우되지 않게) / blockRef는 step 스키마에 넣지
  않음(닫힌 유니온 — zod strip이 왕복에서 제거) / 블록 캡처 v1은 시퀀스 전체 캡처.
- **스모크 검증**: 서버 실부팅(8791) — setup→사용자 추가→잘못된 PIN 401→작업 CRUD→
  409 충돌 봉투→잠금→run append(operator 세션 강제 교체 확인)→stats→soft-delete/restore
  전부 통과. 브라우저 게이트(falling-boxes)로 서버 부재 시 로컬 모드 부팅 무회귀 확인.

### Phase 12 통합 글루 리뷰 반영 (독립 code-reviewer 패스)

통합 글루(`src/ui/console/glue.ts` + `main.ts` 변경분)만 워크플로 검증 이후에 작성돼
독립 리뷰를 받지 않은 상태였다. 리뷰 결과 critical 1 · major 10 · minor 19를 받아
데이터·잠금 손실 계열과 무음 no-op 계열을 전부 수정했다. 주요 결정:

- **사본 저장 성공 확인 후에만 충돌 레코드를 지운다** (C-1). `createTaskAndOpen`이
  `Promise<boolean>`을 반환하도록 바꿨다 — 실패(401·5xx) 시 그 레코드가 오프라인
  편집분의 유일본이므로 지우면 복구 경로가 사라진다.
- **재시도 가능한 실패에서 실행 기록을 버리지 않는다** (M-1·M-2). `unauthorized`와
  5xx는 대기열에 남기고 중단한다 — `OfflineOutbox.flush`의 중단 정책과 동일하게 맞췄다.
  4xx 스키마 거부만 폐기한다(재시도해도 같다). 대기열에 상한(500건)도 뒀다.
- **잠금 acquire의 await 경계에서 컨텍스트 동일성을 검사한다** (M-3). 작업 A→B를 빠르게
  연달아 열면 A의 늦은 응답이 B 컨텍스트에 lockHeld를 찍고 heartbeat 타이머를 덮어써
  **A의 잠금이 영원히 갱신되는**(타인이 못 여는) 누수가 났다. 낡은 응답은 즉시 반납한다.
- **`OfflineOutbox.dropPendingUpdates` 신설** (M-5). 오프라인에서 같은 문서를 두 번
  저장하면 두 op가 같은 baseVersion을 갖고, 재연결 flush에서 **자기 자신과 409**가 나
  사용자의 최신본이 "다른 사용자가 먼저 저장했습니다"로 격리됐다. 로컬은 마지막 저장이
  이기는 것이 의도이므로 이전 update만 대체한다(create/remove/restore는 의미가 달라 보존).
  회귀 테스트가 "드롭 없으면 conflictCount 1, 있으면 0 + 최신본만 전송"을 고정한다.
- **셸을 연결 모드 판정 후에 마운트한다** (M-7). 즉시 마운트하면 기본 라우트(#/tasks)
  화면이 판정 전에 API를 발사하고, 정적 배포에서 그 응답이 상태 머신을 "서버 도달"로
  되돌려 **서버가 없는데 온라인 배지가 뜨는** 고착이 생겼다. 목록 화면 3종에도
  `mode === 'local'`이면 요청 자체를 보내지 않는 가드를 넣었다(blocks 화면 관례 확산).
- **`currentUserId`를 getter로** (M-8). 화면은 셸이 1회만 마운트하므로 값 스냅샷이면
  로그인 전 null로 고정돼 **내 잠금이 "○○님 편집 중"으로** 보였다.
- **속도 기본값을 실제로 적용한다** (M-9). `orchestrator.setSpeed` + 재생 바 표시를
  함께 맞춘다(`setSpeedDisplay` 신설) — select만 2×인 조용한 no-op이었다.
- **`sequence: null`을 경계에서 undefined로 정규화** (M-10). buildScene의 관문이
  `!== undefined`라 null이 validateSequence로 들어가 **새 작업을 만들 때마다** 빨간
  "시퀀스 검증 실패" 토스트가 떴다.
- 그 외: 저장 in-flight 가드(Ctrl+S 연타 자기-409), 기준선을 **전송한 스냅샷**으로
  (왕복 중 편집 유실 방지), 사용자 전환 시 잠금·컨텍스트 해제(공유 단말에서 B가 A의
  작업에 저장하는 경로), 공정 저장의 오프라인 큐 적재, 덮어쓰기의 임의 base 폴백 제거,
  완주 후 ⏭의 유령 런 기록 방지, 블록 파라미터 빈 칸→NaN 승격(0 오삽입 방지),
  글루 모달 열림 동안 스튜디오 단축키 정지, 편집분 없는 충돌의 비활성 사유 표기,
  터치 타깃·표면 토큰 사용, 죽은 상수 제거.

**실브라우저 스모크 추가**: 서버(8792) + 실제 크로미움으로 부트 → 셋업 마법사 → PIN →
작업 목록 → 새 작업 생성 → 스튜디오 전환 → 목록 복귀까지 전부 통과(페이지 오류 0).
이 과정에서 **연결 배지가 72px 레일 폭을 넘어 잘리는** 시각 결함을 발견해 고쳤다 —
레일은 두 모드 모두 좁으므로 배지는 항상 compact(아이콘 + 대기 건수)이고 전체 문구는
title/aria-label로 남는다.

## Phase 13 — 시퀀스 JSON 직접 편집 + 버전 이력 (2026-08-10)

`{} JSON` 패널이 읽기 전용이라 **무손실 변환(`toSequence`/`fromSequence`)이 내부에만
존재하고 사용자가 텍스트로 편집할 입구가 없었다.** 그래프→JSON은 즉시 반영되는데
JSON→그래프는 파일 업로드(문서 전체 재로드)뿐이라 비대칭이었다.

### 결정과 근거

- **패널은 텍스트만 다룬다.** 파싱·스키마 검증·그래프 커밋은 전부 통합자(main.ts)가
  `applyJson`에서 수행하고 `{ok} | {errors}`로 돌려준다. 패널이 스키마를 알면 §2.9
  게이트가 두 곳으로 갈라진다 — 검증의 단일 지점을 지킨다.
- **전체 교체도 노드 op와 같은 게이트를 지난다**(`replaceSequenceFromValidated`):
  `validateSequence(스키마+씬 참조) → fromSequence → serializeGraph(재직렬화 왕복) →
  commit`. 마지막 재직렬화는 형식적으로 보이지만 §2.8("편집 결과는 항상 직렬화 가능")을
  기계적으로 보증한다 — **사람이 쓴 JSON이 들어오는 유일한 경로**라 더 필요하다.
- **실패 시 텍스트를 지우지 않는다.** 검증 거부는 인라인 한국어 사유만 띄우고 편집본은
  그대로 둔다. 고쳐 쓸 수 있어야지, 지워버리면 처음부터 다시 타이핑이다.
  파싱 오류는 엔진 메시지의 `position N`을 **줄 번호로 환산**해 안내한다.
- **편집 중 외부 변경은 덮어쓰지 않는다.** 캔버스/인스펙터 커밋으로 refresh()가 와도
  dirty면 텍스트를 보존하고 "밖에서 바뀌었습니다 — [현재 값 불러오기]" 배너만 띄운다.
  30초 타이핑을 말없이 날리는 것이 이 패널에서 가능한 가장 나쁜 일이다.
  같은 이유로 **편집 중 Escape는 패널을 닫지 않는다**(닫기는 명시 버튼).
- **버전 스택은 Undo와 별도 축이다**(`ui/sequence-versions.ts`). Undo(씬+시퀀스 전체
  재로드)는 (a) 몇 번 눌러야 그때로 가는지 안 보이고 (b) 건너뛸 수 없고 (c) 씬은 두고
  시퀀스만 되돌릴 수 없다 — JSON 편집에서 가장 자주 필요한 조작이 (c)다.
- **되돌리기도 새 버전으로 append**된다. 되돌리기를 되돌릴 수 있어야 실수의 값이 0이 된다
  (§2.11). 그래서 "직전 버전과 내용이 같으면 기록하지 않는다"는 중복 억제는 **직전 하나만**
  본다 — 과거와 같은 내용이어도 새 버전이 쌓인다.
- **라벨은 이전/이후 비교로 자동 도출**한다(`describeSequenceChange`). 편집 호출부
  (캔버스 드래그·인스펙터 폼·파사드·재동기)가 저마다 라벨을 넘기게 하면 한 곳만
  빠뜨려도 이력에 '알 수 없음'이 남는다. 호출부를 하나도 건드리지 않았고, 새 편집 경로가
  생겨도 자동으로 이름이 붙는다. JSON 직접 편집/되돌리기만 명시 라벨을 준다.
- **세션 한정(메모리)** — 새로고침 후 복구는 IndexedDB 자동저장 초안이, 영구 보존은
  파일/서버 저장이 이미 담당한다. 축을 셋으로 갈라 저장소를 늘리는 대신 UI가 한계를
  명시한다("이 이력은 이 세션에만 유지됩니다"). cap 50, 초과 시 오래된 것부터 폐기하고
  폐기된 버전 번호는 재사용하지 않는다(목록의 안정 식별자).
- `flowSeqMeta`는 **제자리 갱신**한다 — 다른 클로저들이 이 객체를 캡처하고 있어 재대입은
  조용히 낡은 참조를 남긴다. 사람이 쓴 JSON이 id/loop를 바꿀 수 있어 진실을 따라간다.

### 검증

단위 19건 신규(버전 스택 수명·라벨 도출·상한·깊은 복사·clear). 실브라우저 스모크로
①형식 오류 줄 안내 + 시퀀스 불변 ②스키마 위반 거부 + 시퀀스 불변 ③유효 편집 반영
(7→6) ④그래프→JSON 재동기 ⑤버전 라벨 기록 ⑥v1 되돌리기(6→7) ⑦되돌리기가 새 버전으로
append ⑧되돌린 시퀀스가 검증 통과 상태 — 전부 통과, 페이지 오류 0.

### Phase 13 리뷰 반영 (독립 code-reviewer 패스)

MAJOR 5건 · MINOR 12건을 받아 전부 또는 판단 근거와 함께 처리했다. §2.8/§2.12 게이트
자체는 뚫린 곳이 없다는 확인을 받았고(커밋 대상이 `validated.value`가 아니라 왕복 결과인
`serialized.sequence`라는 점이 핵심), 나머지는 상태 관리·수명 결함이었다.

- **`baselineText`와 `editor.value`는 항상 짝을 맞춘다**(M-1). 시퀀스 없는 씬
  (falling-boxes 등)을 지나면 null 분기가 기준선만 ''로 낮추고 textarea 내용을 남겨
  **영구 dirty**가 됐다. 그 결과 (a) Escape가 죽고 (b) 이후 모든 refresh가 stale 배너로
  빠져 **시퀀스가 있는 씬인데 "시퀀스가 없습니다"를 계속 보여줬다.** 불변식을 함수
  주석으로 못 박았다.
- **teardown이 씬 수명 클로저를 끊는다**(M-2). `applyJsonToSequence`/
  `restoreSequenceVersion`을 null로 되돌리지 않아, 빌드 실패(손상 파일)나 전환 중
  (URDF 로딩 수백 ms)에 [적용]/[되돌리기]가 **해제된 씬에 커밋**됐다 — 아무 일도
  없었는데 성공 토스트가 뜨고 죽은 스냅샷이 Undo 스택에 들어갔다.
- **버전 스택을 앱 수명으로 올렸다**(M-3). 씬 수명이면 Ctrl+Z(전체 재빌드)가 이력을
  통째로 지워, **이 기능의 존재 이유(안전망)를 다른 되돌리기 축이 파괴**했다. 대신
  `loadScene`이 `!opts.fromHistory`일 때만 `clear()`한다 — `history.reset()`과 같은
  조건이다. Undo 복원 빌드는 '되돌리기(Ctrl+Z)로 복원' 라벨로 이력에 남는다.
- **확인 바는 언제나 하나**(M-4). 스칼라 `pendingRestore`가 직전 바를 잊어 서로 다른
  버전을 가리키는 확인 바가 동시에 뜰 수 있었다. DOM을 진실로 삼아 정리하고, 같은 행
  재클릭은 토글로 만들었다(조용한 no-op 제거 — §6).
- **탭에 roving tabindex**(M-5). `role="tablist"`만 붙이면 `shortcuts.ts`의
  `widgetOwnsKey`가 ←/→를 위젯 소유로 판정해 라우터를 양보시키는데 정작 핸들러가 없어
  **어디서도 처리되지 않는 사장 키**가 됐다. dock.ts의 tablist/tab/tabpanel + roving
  배선을 그대로 따랐다.
- **`jsonErrorKo`를 실제 엔진 출력에 맞춰 다시 썼다.** 최근 V8은 `at position N`을 주지
  않는다(`Unexpected token '}', ..."c":\n}" is not valid JSON`) — 원래 구현의 줄 번호
  환산이 **실제로는 거의 동작하지 않았다.** position / `line N`(Firefox) / 인용 발췌
  위치 / 입력 조기 종료 네 경로를 모두 처리하고, 어느 것도 안 맞으면 영문 원문을
  노출하는 대신 한국어로 무엇을 볼지 말한다(§4-b). 테스트는 **실제 `JSON.parse` 오류
  객체**로 고정한다(메시지 형식을 추측하지 않는다).
- Tab 들여쓰기를 `execCommand('insertText')`로 바꿔 **textarea의 네이티브 undo 스택을
  보존**한다(대입은 스택을 파기해 Ctrl+Z가 죽는다 — 전역 Ctrl+Z는 텍스트 입력을 제외하므로
  브라우저 undo가 편집기 안의 유일한 축이다). 실패 시 대입 폴백.
- 안내 문구가 거짓말하지 않게 `record`의 실제 기록 여부를 반환해, 공백·키 순서만 바꾼
  적용에는 "새 버전은 만들지 않았습니다"라고 말한다.
- WCAG 2.5.3(Label in Name) — 확인 버튼의 title이 보이는 라벨('되돌리기')을 포함한다.
  §4-b 규칙 3 — 도메인 원문(`ControlSequence`, JSON 본문)에 `lang="en"`.
- `runFlowOp`의 죽은 `versionLabelKo` 파라미터 제거(라벨은 자동 도출이 담당).
- 순수 헬퍼 테스트 10건 신규(DoD §8) — `jsonErrorKo` 4형식 + 클램프 + 원문 미노출.

**게이트 수정**: `--expect=flow-graph`의 (f)가 읽던 `[data-testid="json-content"]`(`<pre>`)는
편집 모드에서 숨겨진다. 진실인 textarea `value`를 읽고 `<pre>`는 폴백으로 둔다.
flow-graph · orchestration · planner 게이트 전부 통과.

### Phase 13 후속 — 전체 게이트에서 잡은 실질 회귀: 얇은 레일이 뷰포트를 좁혔다

Phase 12·13 이후 처음 돌린 전체 게이트(13종)에서 `--expect=two-arms`의
"보이는 몸통을 마우스로 끌면 이동한다 ★ 회귀" 1건이 실패했다. 로봇 드래그는 0.316m로
정상인데 **박스만 0m**였다.

**원인은 마우스 가로채기가 아니었다.** 프로브(`elementFromPoint` + 레이아웃 계측)로
확인한 실제 사슬: 얇은 레일이 워크스페이스를 `left: 48px`로 밀어 뷰포트 캔버스가
좁아졌고 → 박스의 투영 앵커가 캔버스 하단 19px 지점(y=425, 바닥 444)으로 올라왔고 →
아래로 30px 끄는 순간 포인터가 캔버스를 벗어나 드래그가 유실됐다.

**게이트 좌표를 손대지 않고 원인을 고쳤다: 로컬 모드에서는 레일을 감춘다.**
콘솔 6화면은 전부 서버 개체를 다루므로 로컬 모드에서는 빈 화면뿐이고, 거기로 가는
레일은 거짓 어포던스다. 더 중요한 건 `docs/BACKEND.md` §1의 약속이다 — 서버가 없으면
앱이 **이전과 똑같이** 보여야 하는데 레일이 그것을 48px만큼 깨고 있었다.
- `railModeForRoute(name, serverMode)` — 로컬 모드 스튜디오는 `hidden`
- 해시로 콘솔 라우트를 직접 열었다면 로컬에서도 레일을 보여 **돌아갈 길은 남긴다**
- 연결 모드 전환(로컬 ↔ 서버) 시 레일·여백이 따라오도록 라우트를 재적용한다
- `setStudioInset`은 레일이 실제로 보일 때만 여백을 준다

교훈: **전면 오버레이 크롬을 추가할 때 "기존 화면은 그대로"라는 약속은 픽셀 단위로
검증해야 한다.** 이 결함은 단위 테스트·타입·린트를 전부 통과했고 신규 화면 스모크도
통과했다 — 오직 **기존 회귀 게이트**만이 잡았다. 신규 기능을 검증하는 것과 기존 기능이
안 깨졌음을 검증하는 것은 다른 작업이다.

**최종 상태**: 전체 게이트 13종 · 단위 1,590건 · 타입(앱+서버) · 린트 전부 초록.
서버 모드 회귀도 실서버+크로미움으로 확인(콘솔 전체 레일 → 스튜디오 얇은 레일 + 48px).

## Phase 13 후속 — "드래그로 순서를 바꿨는데 실행이 이전 순서다" 조사 결과

사용자 신고를 4-에이전트 병렬 조사 + 적대적 검증으로 추적했다. **파이프라인은 무결했다**
— moveNode·toSequence·serializeGraph·player 모두 정상이고, 파사드 재정렬은 언제나
정확히 반영됐다. 결함은 **드롭이 애초에 커밋되지 않는 것**과 **편집 후 ▶의 의미**에 있었다.

### 1. 드롭 판정을 '커서'로 했다 — 가장 자연스러운 제스처가 무음 no-op (critical)

`insertionIndexAt`은 "커서가 이웃의 **원래 중심**을 넘었는가"로 판정했다. 그런데 사용자가
조준하는 것은 커서가 아니라 **고스트**이고, 고스트는 잡은 지점만큼 커서에서 떨어져
그려진다. 노드 라벨이 왼쪽에 있어(`아이콘 x+13, 라벨 x+38`) 사람은 노드 왼쪽 1/4을 잡는다.
그러면 고스트를 이웃 위에 **픽셀 단위로 정확히 겹쳐도** 커서는 아직 이웃 중심에 못 미쳐
아무 일도 일어나지 않는다.

실측(arm-and-boxes, 줌 0.589, 화면 피치 131.9px): 한 칸 이동에 필요한 드래그가
그랩 10%에서 **173px**(고스트 정렬은 132px) — 41px 초과 요구. 그랩 50%는 정확히 경계라
132px에서도 no-op. 데드존 폭은 항상 **2피치**(월드 448px)였다. 두 칸 이동은 **항상 한 칸
앞**에 떨어졌다.

→ 판정을 **고스트 중심 → 최근접 슬롯**(`nearestSlotIndexAt`)으로 바꿨다. **보이는 것이
곧 결과**가 되어 조준과 커밋이 어긋날 수 없고, 데드존이 2피치 → 1피치(제자리 ±반 칸)로
줄었다. 프리뷰와 커밋이 영원히 같은 식을 쓰도록 `dropIndexFor` 한 함수로 묶었다.
기존 `insertionIndexAt`/`reorderTargetIndexAt`은 순수 함수 계약이라 그대로 두고 호출부만
교체했다(테스트 82건 유지 + 신규 6건).

### 2. 무음 no-op — 조용히 되돌리면 "드래그가 먹히지 않았다"로 읽힌다 (major)

`if (finalIndex === d.nodeIndex) { render(); return; }`가 토스트도 announce도 없이
원위치 복원만 했다. 드래그 중에도 삽입선은 순서가 바뀔 때만 보여, 데드존 안에서는
"아무 일도 안 일어나는 중"이라는 신호가 **전혀** 없었다. `onReorderNoop`을 추가해
"순서가 그대로입니다 — 옮길 자리의 노드 위로 끌어다 놓으세요"를 띄운다 (§6).

### 3. 페인 밖 드롭이 조용히 커밋됐다 (major)

노드를 3D 뷰포트로 끌어 올렸다 놓으면 취소가 아니라 **클램프된 좌표로 재정렬**됐다.
Esc와 같은 계약(취소)으로 바꾸고 `onReorderCancelled`로 사유를 알린다.

### 4. 편집 커밋이 씬을 되감지 않아 ▶가 이전 런에서 이어졌다 (critical)

완주 후 재정렬하고 ▶를 누르면 로봇이 **이전 런의 끝 포즈에서** 시작했다. 그 결과
moveJoints 목표가 이미 달성돼 **관절 변화 0.0000 rad로 durationSec 동안 정지**하거나
(샘플 6종 중 4종의 마지막 노드가 정확히 이 조건), waitForCollision이 잔여 접촉으로 즉시
통과하거나 반대로 타임아웃까지 6초를 셌다. 사용자에겐 정확히 "순서를 바꿨는데 로봇이
예전처럼 군다"로 보인다. 같은 ▶ 버튼이 "편집했으면 이어서 / 아니면 처음부터"로 의미가
갈렸다.

→ `sequenceDirtySinceRun` 플래그로 **"새 런 시작"의 판정을 하나로 모았다**: 완주 후 ▶
**또는** 마지막 재생 이후 편집됨 → ⏹ → ▶와 같은 경로(씬·충돌 이력·player 전부 되감기).
⏭ Step도 같은 분기를 탄다. 토스트도 "씬은 유지"라는 옛 계약 대신 "▶ 재생하면 처음부터
새 순서로 실행합니다"로 고쳤다 — 문구가 실제 동작과 일치해야 한다.

### 5. 재생 중 재정렬 시 엔진만 살아 로봇이 얼어붙었다 (major)

시퀀스만 unarm하고 엔진은 playing으로 뒀더니 **로봇은 멈췄는데 오버레이는 Idle**인
모순 상태가 됐다. `engine.state === 'playing'`이면 `orchestrator.pause()`를 함께 부른다.

### 6. 드래그 중 재렌더가 고스트를 영구 소멸시켰다 (major)

`drawWorld()`의 `replaceChildren`이 고스트/삽입선을 DOM에서 떼어내도 모듈 변수는
non-null로 남아, 이후로는 **떨어져 나간 요소에 속성만 계속 썼다**. 드래그 중 휠 줌으로
LOD 임계를 넘으면 고스트·삽입선·원본 흐림이 전부 사라져 "취소된 것처럼" 보이는데
드롭은 그대로 커밋됐다. `isConnected` 확인으로 재생성한다.

### 7. 드롭 시 fromIndex를 nodeId로 재조회한다

`d.nodeIndex`는 pointerdown 시점 스냅샷이라, 드래그 도중 외부 편집(JSON 적용·버전
되돌리기)이 그래프를 바꾸면 ±1 어긋났다. 드롭 시점에 다시 찾는다.

### 게이트 결정론 수정

`--expect=flow-graph`가 **개발자가 백엔드를 띄워 뒀는지에 따라** 실패했다 — vite preview가
vite.config의 프록시를 그대로 써서 앱이 서버 모드로 부팅하고 **로그인 화면이 캔버스를 덮어**
모든 클릭이 막힌다. 게이트가 주변 상태에 좌우되면 안 되므로 `page.route('**/api/**', abort)`로
로컬 모드를 명시적으로 강제한다(이 게이트의 검증 대상이 곧 서버 없는 경로다).

### 남긴 것 (별도 작업)

- moveJoints 무동작(목표 == 현재 포즈) 시 UI가 'active'로만 칠해 "왜 안 움직이는지" 안 보인다
  → `NodeRunStatus`에 무동작 배지 추가 제안(조사 finding #3).
- 비활성 노드가 실행된 노드와 같은 'done'으로 칠해진다 → 'skipped' 상태 분리 제안.
- 무한 goto 뒤로 옮긴 노드의 도달 불가 경고(정적 판정) 제안.
